...
| 24 | 2026-09 | **FUNDING-CARRY** (Krypto-Perpetuals, delta-neutral): erster Test einer *strukturellen Zahlung* statt Vorhersage. **Neue Datenquelle** (ccxt-Funding-Historie, öffentlich) + **neuer Mechanismus**. Longs zahlen Shorts Funding → delta-neutral (Spot long/Perp short) kassiert es ohne Kursrisiko. Pro Coin + diversifizierter Korb durch die volle Batterie, Kosten-Sweep. WICHTIG: ✅ hier = Carry war in-sample dauerhaft positiv, NICHT risikofrei (Börsen-/Liquidations-/Regime-Risiko unerfasst); auf Alpaca nicht handelbar (nur Forschung) | _(läuft auf VM — Ergebnis in `experiment24_results.md`)_ | ⏳ |
